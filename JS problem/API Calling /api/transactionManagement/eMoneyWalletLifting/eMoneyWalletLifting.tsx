...
const SYSTEM_LIFTING_URL = "api/payment/v1/moneyMovement"

export function fetchSystemLiftingHistory(data: any) {
  return httpRequest(publicContext()).post(
    `${SYSTEM_LIFTING_URL}/fetchListLiftingHistory`,
    data,
  )
}
...
export function fetchBankNames(body: any) {
  return httpRequest(publicContext()).post(
    "/api/payment/v1/moneyMovement/fetch-bank-details",
    body,
  )
}

export function fetchEMoneyLiftingEntity(body: any) {
  return httpRequest(publicContext()).post(
...
export function searchUserData(body: any) {
  return httpRequest(publicContext()).post(
...
export function getWalletDetailsData(body: any) {
  return httpRequest(publicContext()).post(
    "/api/sysportal/v1/system-commons/properties",
    body,
  )
...
export const downloadTransactionDocument = async (
  ...
  axios({
    ...
    responseType: "blob",
    data: { ...body },
    headers: { ...getHeaderWithApiKey(body), responseType: "blob" },
  }).then((response) => {
...
export function validateLiftingRequest(data: any) {
  return httpRequest(authContext()).post(
    `${SYSTEM_LIFTING_URL}/validateLiftingRequest`,
    { ...data },
  )
}

export function liftingPayment(data: any) {
  return httpRequest(authContext()).post(
    `${SYSTEM_LIFTING_URL}/liftingPayment`,
    data,
    {
      headers: { channel: "BO" },
    },
...
export const fetchUploadedFile = async ({ fileUrl, fileType }: any) => {
  try {
...
export function TransactionDetailView(body: any) {
  return httpRequest(authContext()).post(
